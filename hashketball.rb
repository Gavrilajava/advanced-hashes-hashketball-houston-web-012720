# Write your code here!
require 'pry'

def game_hash
  {
    home: {
      team_name: "Brooklyn Nets",
      colors: ["Black", "White"],
      players: [
        {
          player_name: "Alan Anderson",
          number: 0,
          shoe: 16,
          points: 22,
          rebounds: 12,
          assists: 12,
          steals: 3,
          blocks: 1,
          slam_dunks: 1
        }, {
          player_name: "Reggie Evans",
          number: 30,
          shoe: 14,
          points: 12,
          rebounds: 12,
          assists: 12,
          steals: 12,
          blocks: 12,
          slam_dunks: 7
        }, {
          player_name: "Brook Lopez",
          number: 11,
          shoe: 17,
          points: 17,
          rebounds: 19,
          assists: 10,
          steals: 3,
          blocks: 1,
          slam_dunks: 15
        }, {
          player_name: "Mason Plumlee",
          number: 1,
          shoe: 19,
          points: 26,

          rebounds: 11,

          rebounds: 12,

          assists: 6,
          steals: 3,
          blocks: 8,
          slam_dunks: 5
        }, {
          player_name: "Jason Terry",
          number: 31,
          shoe: 15,
          points: 19,
          rebounds: 2,
          assists: 2,
          steals: 4,
          blocks: 11,
          slam_dunks: 1
        }
      ]
    },
    away: {
      team_name: "Charlotte Hornets",
      colors: ["Turquoise", "Purple"],
      players: [
        {
          player_name: "Jeff Adrien",
          number: 4,
          shoe: 18,
          points: 10,
          rebounds: 1,
          assists: 1,
          steals: 2,
          blocks: 7,
          slam_dunks: 2
        }, {
          player_name: "Bismack Biyombo",
          number: 0,
          shoe: 16,
          points: 12,
          rebounds: 4,
          assists: 7,

          steals: 22,

          steals: 7,

          blocks: 15,
          slam_dunks: 10
        }, {
          player_name: "DeSagna Diop",
          number: 2,
          shoe: 14,
          points: 24,
          rebounds: 12,
          assists: 12,
          steals: 4,
          blocks: 5,
          slam_dunks: 5
        }, {
          player_name: "Ben Gordon",
          number: 8,
          shoe: 15,
          points: 33,
          rebounds: 3,
          assists: 2,
          steals: 1,
          blocks: 1,
          slam_dunks: 0
        }, {
          player_name: "Kemba Walker",
          number: 33,
          shoe: 15,
          points: 6,
          rebounds: 12,
          assists: 12,

          steals: 7,

          steals: 22,

          blocks: 5,
          slam_dunks: 12
        }
      ]
    }
  }
end

def allplayers
  
  players = game_hash[:away][:players]
  players += game_hash[:home][:players]
end

def player(player_name)
  allplayers.find { |player|
    player[:player_name] == player_name
  }
end

def teams
  team_array = []
  team_array << game_hash[:home]
  team_array << game_hash[:away]
  team_array
end

def get_team(name)
  teams.find { |team|
    team[:team_name] == name
  }
end

def test
  allplayers.each { |player|
    name = player[:player_name]
    points_scored = num_points_scored(name)
    shoesize = shoe_size(name)
    #puts name
    puts "#{name}: points scored: #{points_scored}"
    #puts "shoe_size: #{shoesize}"
  }
end

def test2
  puts player_numbers("Charlotte Hornets")
  
end


def num_points_scored(player_name)
  player(player_name)[:points]
end

def shoe_size(player_name)
  player(player_name)[:shoe]
end

def team_colors(team_name)
  team = teams.find { |team|
    team[:team_name] == team_name
  }
  team[:colors]
end

def team_names
  names = []
  teams.each { |team|
    names << team[:team_name]
  }
  names
end

def player_numbers(team_name)
  
  numbers = get_team(team_name)[:players].map { |player|
    player[:number]
  }
  numbers.sort
end



def player_stats(player_name)
  stats = player(player_name)
  stats.delete(:player_name)
  stats
end

def big_shoe_rebounds
  bigfoot = allplayers.max_by{ |player|
    player[:shoe]
  }
  bigfoot[:rebounds]
end

def most_points_scored
  scorer = allplayers.max_by { |player|
    player[:points]
  }
  scorer[:player_name]
end

def most_steals
  scorer = allplayers.max_by { |player|
    player[:steals]
  }
  scorer[:player_name]
end

def player_with_longest_name
  scorer = allplayers.max_by { |player|
    player[:player_name].length
  }
  scorer[:player_name]
end

def long_name_steals_a_ton?
  player_with_longest_name == most_steals
end

def winning_team
  winner = teams.max_by { |team|
    team[:players].sum { |player|
      player[:points]
    }
  }
  winner[:team_name]
end

def test3
  puts winning_team
end


def num_points_scored(player_name)
  player(player_name)[:points]
end

def shoe_size(player_name)
  player(player_name)[:shoe]
end

def team_colors(team_name)
  team = teams.find { |team|
    team[:team_name] == team_name
  }
  team[:colors]
end

def team_names
  names = []
  teams.each { |team|
    names << team[:team_name]
  }
  names
end

def player_numbers(team_name)
  
  numbers = get_team(team_name)[:players].map { |player|
    player[:number]
  }
  numbers.sort
end

def player_stats(player_name)
  
end


